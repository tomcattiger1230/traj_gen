...
'''
Author: Wei Luo
Date: 2020-08-04 09:05:49
LastEditors: Wei Luo
LastEditTime: 2020-09-11 13:33:10
Note: Note
'''
...
from traj_gen import poly_trajectory_mosek as pt
import time

if __name__ == '__main__':
    dim = 3
    knots = np.array([0.0, 2.0, 4.0, 5.0, 7.0])
    # knots = np.array([0.0, 4.0, 6.5, 8.0, 10.0])
    order = 8
    ...
    # 2. Pin
    ts = knots.copy() # np.array([0.0, 2.0, 4.0, 5.0, 7.0])
    ...
    #                 [0.4, 0.4, .5,]
    #                ])
    # Xdot = np.array([0.2, 0, 0])
    # Xddot = np.array([0, 0, 0])
    # Xenddot = np.array([-0.1, -0.1, 0])
    # Xendddot = np.array([0, 0, 0])
    # # create pin dictionary
    # for i in range(Xs.shape[0]):
    #     pin_ = {'t':ts[i], 'd':0, 'X':Xs[i]}
    #     pTraj.addPin(pin_)
